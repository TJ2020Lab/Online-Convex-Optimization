...
function [reg_max] = iclr_ltc_REAL(m,n,iter,rho,...
...
for i = 1 : n
    for j = 1 : n
        f_val(i) = f_val(i) +...
            (1/2)*( D_feature(1,:,j)*x_dec(:,i) - D_label(1,j) )^2+...
...
% convex
c_eta_step = 1;  % full
c_eta_B_step = 1;  % one point bandit
c_eta_TB_step = 1;  % two point bandit
% strongly convex
s_eta_step = 0.2;  % full
s_eta_B_step = 0.2;  % one point bandit
s_eta_TB_step = 0.2;  % two point bandit
...
for t = 2 : iter
    ...
    for j = 1 : n
        ...
        subg = ( D_feature(t,:,j)*x_dec(:,j) - D_label(t,j))*D_feature(t,:,j)'+...
            2*rho*x_dec(:,j);
        ...
        subg_ban1 = ( (1/2)*( D_feature(t,:,j)*x_dec_t1 - D_label(t,j) )^2 +...
            rho*norm(x_dec_t1,2)^2 )*(m/(delta))*ran_vec;
        
        % compute the TWO-point feedback
        x_dec_t2 = x_dec(:,j) - delta*ran_vec;
        subg_ban2 = (( (1/2)*( D_feature(t,:,j)*x_dec_t1 - D_label(t,j) )^2 +...
            rho*norm(x_dec_t1,2)^2 ) - ((1/2)*( D_feature(t,:,j)*x_dec_t2 - D_label(t,j) )^2 +...
    ...
    for i = 1 : n
        for p = 1 : n
            f_val(i) = f_val(i) +...
                (1/2)*( D_feature(t,:,p)*x_dec(:,i) - D_label(t,p) )^2+...
                rho*norm(x_dec(:,i),2)^2;
