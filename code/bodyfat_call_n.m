...
function [Reg_F_ave,Reg_B_ave,Reg_TB_ave] = bodyfat_call_n(n,T,flag_c,data_set)
% read the data
if data_set==1
...
%         end
      ...
      %  wm of k-regular
        ...
        % compute the regrets
        t = T;
        [Reg_F] =  iclr_ltc_REAL(m,n_i,t,rho,...
            B_randperm(1:t,:,:),A_randperm(1:t,:),f_opt(t),...
            c,flag_c, 0,0,0,wm(:,:,1:t),R);
        [Reg_B] = iclr_ltc_REAL(m,n_i,t,rho,...
            B_randperm(1:t,:,:),A_randperm(1:t,:),f_opt(t),...
            c,flag_c,delta(1,t),xi(1,t),1, wm(:,:,1:t),R);
        [Reg_TB] = iclr_ltc_REAL(m,n_i,t,rho,...
            B_randperm(1:t,:,:),A_randperm(1:t,:),f_opt(t),...
            c,flag_c,delta(2,t),xi(1,t),2, wm(:,:,1:t),R);
        Reg_F_ave(n_i) = Reg_F_ave(n_i) + Reg_F;
        Reg_B_ave(n_i) = Reg_B_ave(n_i) + Reg_B;
        Reg_TB_ave(n_i) = Reg_TB_ave(n_i) + Reg_TB;
    end
end
Reg_F_ave = Reg_F_ave / 5;
Reg_B_ave = Reg_B_ave / 5;
Reg_TB_ave = Reg_TB_ave / 5;
end
